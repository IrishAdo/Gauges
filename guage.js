var Guage = {
	...
	DrawLabelsAndTicks:function(ctx,id){
		...
		for(var index=0; index<maxIndex;index++){
			...
			ctx.rotate(Guage.DegreeToRadians(45+(Guage.singleDegree*(stepPercentage*index))));
		    ctx.stroke(path);
		    ctx.restore();
		    ...
		    //ctx.scale(-1, 1);
			ctx.font = "12px serif";
   // 		 	ctx.rotate(Guage.DegreeToRadians(50));
			// ctx.fillText(Guage.list[id].labels[index], 0, 110);
			// ctx.restore();
			ctx.translate(125,125);
  			ctx.rotate(Guage.DegreeToRadians(45+(Guage.singleDegree*stepPercentage*index)));
